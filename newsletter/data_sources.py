...
class Story:
    ...
    def published_rank(self) -> float:
        ...
        except (OverflowError, OSError):  # pragma: no cover - defensive
            return 0.0
...
def sort_stories(stories: Iterable[Story]) -> List[Story]:
    return sorted(stories, key=lambda item: item.published_rank(), reverse=True)
