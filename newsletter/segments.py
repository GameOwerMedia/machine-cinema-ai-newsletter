...
def _plan_segments(config: Config, stories: List[Story], per_bucket: int) -> List[Section]:
    ...
    for bucket_name, meta in config.newsletter.buckets.items():
        ...
        ranked = sorted(
            stories,
            key=lambda story: (_score(bucket_name, story, keywords), story.published or 0),
            reverse=True,
        )
        chosen, used_domains = _pick_with_diversity(ranked, per_bucket, used_links)
        sections.append(
            Section(title=str(meta.get("label", bucket_name.title())), stories=chosen, style=str(meta.get("style", bucket_name)))
        )
    return sections
...
def _plan_top(config: Config, stories: List[Story], limit: int) -> Section:
    ...
    ranked = sorted(
        stories,
        key=lambda story: (
            max((_score(bucket, story, keywords) for bucket, keywords in bucket_keywords.items()), default=0.0),
            story.published or 0,
        ),
        reverse=True,
    )
